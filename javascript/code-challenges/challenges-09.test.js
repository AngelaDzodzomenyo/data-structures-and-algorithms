...
const maxInArray = (arr) => {
  return arr.reduce((accumulator, value) => {
    let maxVal = Math.max(accumulator, value);
    return maxVal;
  }, 0);
};
...
const checkValues = (obj, value) => {
  const inTheObject = Object.values(obj).includes(value);
  return inTheObject;
};
...
const hasChildrenValues = (arr, character) => {
  ...
  arr.forEach(name => {
    if(Object.values(name).includes(character) && Object.keys(name).includes('children')) {
      hasChildren = true;
    }
  });
  return hasChildren;
