...
const screenForNames = (arr) => {
  let screen = /^(?:Mrs|Mr|Dr|Ms)+.(?:\s)([A-Z][a-z]+)/g;
  return arr.filter(string => screen.test(string));
};
...
const toTitleCase = (arr) => arr.map(string => string.charAt(0).toUpperCase() + string.slice(1));
